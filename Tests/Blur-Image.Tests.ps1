Describe 'Blur-Image' {
    ...
    It 'blurs an image' {
        ...
        $dest = Join-Path $TestDir 'blur.jpg'
        Blur-Image -FilePath $src -OutputPath $dest -Sigma 5
    ...
    It 'blurs an image asynchronously' {
        $src = Join-Path $PSScriptRoot '../Sources/ImagePlayground.Tests/Images/PrzemyslawKlysAndKulkozaurr.jpg'
        $dest = Join-Path $TestDir 'blur-async.jpg'
        Blur-Image -FilePath $src -OutputPath $dest -Sigma 3 -Async
        $orig = [ImagePlayground.Image]::Load($src)
        $img = [ImagePlayground.Image]::Load($dest)
        $img.Width | Should -Be $orig.Width
        $img.Height | Should -Be $orig.Height
